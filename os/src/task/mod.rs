...
impl TaskManager {
    ...
    fn get_current_task_cb(&self) -> TaskControlBlock {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current]
    }
...
pub fn get_current_task_cb() -> TaskControlBlock {
    TASK_MANAGER.get_current_task_cb()
}
...
/// Add time of syscall with 'syscall_id' for current task
